...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof projectType !== 'string' || !VALID_PROJECT_TYPES.includes(projectType)) return NextResponse.json({ error: 'Tipo de proyecto inválido' }, { status: 400 })
    ...
    if (process.env.LEADS_WEBHOOK_URL) {
      try {
        await fetch(process.env.LEADS_WEBHOOK_URL, {
          ...
          body: JSON.stringify({ created_at: new Date().toISOString(), status: 'nuevo', name: name.trim(), email: email.trim(), phone: phone?.trim() || '', projectType, message: message.trim(), attribution: attribution || {} }),
